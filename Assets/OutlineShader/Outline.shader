    ...
    SubShader
    {
        ...
        Pass
        {
			...
			float4 Frag(Varyings i) : SV_Target
			{
				...
				// angle between the normal and the camera (inverted)
				float3 viewNormal = normal0 * 2 - 1;
				float NdotV = 1 - dot(viewNormal, -i.viewSpaceDir);

				// controls the threshold for normal intesnsity 
				...
				// Uses height and _Time to do cool wavy effect
				float outlineWaveSize = .04;
				float outlinePos = i.vertex.y;
				float outlineWave = (sin(outlinePos / outlineWaveSize + _Time.y) + 1) * .5;

				float4 edgeColor = float4(outlineWave, outlineWave, outlineWave, _Color.a * edge);
